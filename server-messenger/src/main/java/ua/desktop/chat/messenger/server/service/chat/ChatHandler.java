...
public class ChatHandler {
    ...
    public void processInputChoice() throws IOException, OpenSessionException {
        String userInput;
        while ((userInput = clientHandler.getSocketInputReader()
                                         .readLine()) != null) {
            switch (userInput) {
    ...
    public boolean checkCountCompanion(UserDTO user, MessageDTO message, ClientHandler sender) {
        if (connectionHandler.getClientHandlers()
                             .size() <= 1) {
    ...
    public synchronized void sendUserNameList(Multimap<String, ChatDTO> ul) {
        StringBuilder userListResponse = new StringBuilder();
        clientHandler.getSocketOutputWriter()
                     .println(ChatConstant.USERS_COMMAND);
        ...
        for (Map.Entry<String, ChatDTO> entry : ul.entries()) {
            ...
            userListResponse.append(userChat)
                            .append(":")
                            .append(chatDTO.getTypeChat())
                            .append(":")
                            .append(chatDTO.getUser()
                                           .getUsername())
                            .append(",");

            createChatsIfNotExist(userChat, chatDTO, clientHandler.getInitializeUser()
                                                                  .getUserDTO());
        }
        clientHandler.getSocketOutputWriter()
    ...
    public void informAllClientsUserNameList() throws OpenSessionException {
        ...
        for (Map.Entry<String, ClientHandler> key : connectionHandler.getClientHandlers()
                                                                     .entrySet()) {
            ClientHandler client = key.getValue();
            client.getChatManager()
                  .sendUserNameList(userNameAndChatInfo);
        }
    }
    ...
    private void fillChatNameList() {
        ...
        for (Map.Entry<String, ClientHandler> key : connectionHandler.getClientHandlers()
                                                                     .entrySet()) {
            ClientHandler client = key.getValue();
            userNameAndChatInfo.put(ChatConstant.GLOBAL_TYPE, new ChatDTO(TypeChat.GLOBAL, null, client.getInitializeUser()
                                                                                                       .getUserDTO()));
            userNameAndChatInfo.put(client.getInitializeUser()
                                          .getUsername(),
                    new ChatDTO(TypeChat.PRIVATE, client.getInitializeUser()
                                                        .getUserDTO()
                                                        .getId(),
                            client.getInitializeUser()
                                  .getUserDTO()));
        }
    }

    private void fillChatNameListGroup() throws OpenSessionException {
        Multimap<String, ChatDTO> userNameAndChatInfo = connectionHandler.getUserNameAndChatInfo();
        for (Map.Entry<String, ClientHandler> key : connectionHandler.getClientHandlers()
                                                                     .entrySet()) {
            ClientHandler client = key.getValue();
            ...
            List<ChatDTO> chatList = connectionHandler
                    ...
                    .readChatsByType(TypeChat.GROUP, client.getInitializeUser()
                                                           .getUserDTO()
                                                           .getId());
        ...
        new Thread(() -> {
            synchronized (this) {
                try {
                    ...
                    logger.error("The opening session failed!", e);
                }
