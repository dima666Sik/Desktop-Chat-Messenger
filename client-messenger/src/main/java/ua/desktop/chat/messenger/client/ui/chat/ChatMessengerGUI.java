...
public class ChatMessengerGUI extends JDialog {
    ...
    public void addChatList(final Multimap<String, ChatDTO> chats) {
        SwingUtilities.invokeLater(() -> {
            ...
            for (Map.Entry<String, ChatDTO> entry : chats.entries()) {
                if (entry.getValue()
                         .getTypeChat() == TypeChat.PRIVATE && !entry.getKey()
                                                                     .equals(client.getUser()
                                                                                   .getUsername())
                        || (entry.getValue()
                                 .getTypeChat() == TypeChat.GROUP && entry.getValue()
                                                                          .getUser()
                                                                          .getUsername()
                                                                          .equals(client.getUser()
                                                                                        .getUsername()))
                        || (entry.getValue()
                                 .getTypeChat() == TypeChat.GLOBAL && entry.getValue()
                                                                           .getUser()
                                                                           .getUsername()
                                                                           .equals(client.getUser()
                                                                                         .getUsername()))) {
                    model.addElement(String.format("%s [%s]", entry.getKey(), entry.getValue()
                                                                                   .getTypeChat()));
    ...
    public void updateChat(final MessageDTO serverMSG) {
        SwingUtilities.invokeLater(() -> {
            if (serverMSG.getChat()
                         .getUser()
                         .getUsername()
                         .equals(textNameChat) && serverMSG.getChat()
                                                           .getTypeChat() == TypeChat.PRIVATE
                    || serverMSG.getChat()
                                .getNameChat()
                                .equals(textNameChat)
                    && (!serverMSG.getChat()
                                  ...
                                  .equals(client.getUser()
                                                .getUsername()))
                    && (serverMSG.getChat()
                                 .getTypeChat() == TypeChat.GLOBAL
                    || serverMSG.getChat()
                                .getTypeChat() == TypeChat.GROUP)) {
                text.append(serverMSG.getMessage())
                    .append(" \n");
                textArea.append(text.toString());
                clearText();
            }
    ...
    private String buildMessageText(MessageDTO messageDTO) {
        String companionName = messageDTO.getChat()
                                         .getUser()
                                         .getUsername();

        if (messageDTO.getChat()
                      .getTypeChat() == TypeChat.PRIVATE) {
    ...
    private String buildPrivateChatMessageText(MessageDTO messageDTO, String companionName) {
        String senderName = messageDTO.getChat()
                                      .getUser()
                                      .getUsername();
        boolean isCurrentUser = senderName.equals(client.getUser()
                                                        .getUsername());

        String chatName = isCurrentUser ? companionName : messageDTO.getChat()
                                                                    .getNameChat();
    ...
    private void configureMainWindow() {
        ...
        jLabelUserName.setText(client.getUser()
                                     .getUsername());
    }
    ...
    public Map<String, TypeChat> generateMapFromUIDataChatNameAndTypeChat(String nameChat) {
        if (nameChat != null) {
            ...
            String originType = stringList.get(1)
                                          .replace("[", "")
                                          .replace("]", "");
    ...
    private void configureChatDisplay() {
        ...
        textArea.append(String.format("Username [%s] | Host [%s] | Port [%s]%n",
                client.getUser()
                      .getUsername(), client.getHost(), client.getPortNumber()));
    }
    ...
    private void configureSendMessageButton() {
        sendMessage.addActionListener(e -> {
            ...
            if (!textMessage.isEmpty()) {
                ...
                String messageText = buildMessageText(message, textNameChat, client.getUser()
                                                                                   .getUsername(), true);
    ...
    private String buildMessageText(MessageDTO messageDTO, String companionName, String clientUserName, boolean flagSender) {
        ...
        messageText.append(messageDTO.getLocalDateTime()
                                     .format(formatter));

        if (typeChat == TypeChat.GLOBAL) {
            if (!flagSender) {
                messageText.append(")[GLOBAL] ")
        ...
        messageText.append(messageDTO.getMessage())
                   .append("\n");
            ...
            public void windowClosing(WindowEvent e) {
                Thread thread = new Thread(() -> {
                    if (client.getIsConnected()) {
                        ...
                        client.getCommunicationHandler()
                              .setActive(false);
